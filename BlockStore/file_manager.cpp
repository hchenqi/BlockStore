...
Query insert_STATIC_data = "insert into STATIC values (?)";  // blob -> void
Query select_data_STATIC = "select * from STATIC";  // void -> blob
Query update_STATIC_data = "update STATIC set data = ?";  // blob -> void

Query insert_id_OBJECT_gc = "insert into OBJECT (gc) values (?) returning id";  // bool -> uint64
Query select_data_ref_OBJECT_id = "select data, ref from OBJECT where id = ?";  // uint64 -> blob, blob
Query update_OBJECT_data_ref_id = "update OBJECT set data = ?, ref = ? where id = ?";  // blob, blob, uint64 -> void
...
Query select_ref_OBJECT_gc = "select ref from OBJECT where id in (select * from BUFFER) and gc = ?";  // bool -> vector<blob>
...
FileManager::FileManager(const char file[]) : db(file) {
	if (db.ExecuteForOne<uint64>(select_count_TABLE) != table_count) {
		...
		metadata.root_index = CreateBlock();
		metadata.gc_mark = false;
		metadata.gc_phase = GcPhase::Idle;
		db.Execute(insert_STATIC_data, Serialize(metadata).first);
	} else {
		metadata = Deserialize<Metadata>({ db.ExecuteForOne<blob>(select_data_STATIC), {} });
	}
}

void FileManager::MetadataUpdated() {
	db.Execute(update_STATIC_data, Serialize(metadata).first);
}
